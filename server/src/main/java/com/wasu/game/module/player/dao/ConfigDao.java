...
//				query.setLong(0, userId);
	...
	public Config getConfigByPlayerNum(final int key){

		return hibernateTemplate.executeWithNativeSession(new HibernateCallback<Config>() {

			@Override
			public Config doInHibernate(Session session)throws HibernateException, SQLException {

				SQLQuery query = session.createSQLQuery("SELECT * FROM config where config_key like ?");
				query.setString(0,"playerNum_"+key);
				query.addEntity(Config.class);
