...
import com.wasu.game.enums.TypePriorityOfOutFrist;
 ...
 */
@Component
public class CreateTipUtilsOfOutFrist {
    //提示碰的生成
    ...
    public List<Tip> createTip(RoomInf roomInfo, Map<Integer, List<List<Card>>> map, int type) {
        int prePosition=roomInfo.getCurrent();
    ...
    private int getPriority(int type, int position, int operationPrestion) {
        return getPositionPriority(position,operationPrestion)+ TypePriorityOfOutFrist.getTypePriotityByType(type).getPriority();
    }
    /**
    ...
    private Tip getOverTip(RoomInf roomInf,PlayerST player) {
        ...
        for (int i=0;i<roomInf.getPlayers().size();i++){
            ...
            tipInfo.setCards(getCards(roomInf.getPlayers().get(i).getCards()));
            tipInfos.add(tipInfo);
        }
        tip.setTipInfos(tipInfos);
        return tip;
